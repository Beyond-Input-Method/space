...
function M.func(input, seg, env)
  -- 检测是否包含数学运算符或函数
  if not seg:has_tag("jisrqi") then return end
      
  local code = replaceToFactorial(input)
  ...
  if success then
    ...
    yield(Candidate(input, seg.start, seg._end, code .. "=" .. tostring(result), ""))
    
  else
    yield(Candidate(input, seg.start, seg._end, code, "解析失败"))
  end
end
